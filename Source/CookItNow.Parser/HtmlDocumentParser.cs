...
namespace CookItNow.Parser
{
    internal abstract class HtmlDocumentParser : IHtmlParser
    {
        ...
        private Func<CultureInfo, IActionDetector> _actionDetectorFactory;
        private Func<CultureInfo, ITimerDetector> _timerDetectorFactory;
        private Func<CultureInfo, IIngredientDetector> _ingredientDetectorFactory;
        private Func<CultureInfo, IMeasureUnitDetector> _measureUnitDetectorFactory;
        private Func<CultureInfo, ILanguageHelper> _languageHelperFactory;
        private Func<CultureInfo, ISubrecipeRepository> _subrecipeRepositoryFactory;
        ...
        protected HtmlDocumentParser(IHtmlLoader htmlLoader, 
            string baseDomain)
        {
        ...
        public async Task<QuickRecipe> ParseHtmlAsync(Uri uri)
        {
            ...
            if (subrecipeNodes != null && subrecipeNodes.Any())
            {
                ...
                {
                    var subrecipeNode = subrecipeNodes[subrecipeIndex];
            ...
            this._subrecipeRepository.KnownSubrecipes
                .ForEach(x => ClearUnusedSubrecipe(recipe, x.Key));
        ...
        private void InitializeCulture(HtmlDocument document)
        {
            ...
            this._recipeCulture = CultureInfo.GetCultureInfoByIetfLanguageTag(language);
            ...
            ref int ingredientId)
        {
            foreach (var ingredientNode in ingredientNodes)
            {
                ...
                var firstCommaIndex = ingredientPhrase.IndexOf(",", StringComparison.InvariantCulture);
